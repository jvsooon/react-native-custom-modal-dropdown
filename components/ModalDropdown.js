...
export default class ModalDropdown extends Component {
      ...
      >
        {children || (
          <View style={styles.button}>
            ...
            <RightComponent style={[styles.buttonText, buttonTextStyle]}>
                <MaterialIcons name={this.state.arrow} size={24} color="black" />
            </RightComponent>
          </View>
        )}
      </ButtonTouchable>
  ...
  _renderDropdown() {
    ...
    return (
      <FlatList
        {...dropdownListProps}
        data={options}
        scrollEnabled={false}   // self changed from scrollEnabled to false
        style={styles.list}
  ...
  _renderItem = ({ item, index, separators }) => {
    ...
    return <RowTouchable style={{borderRadius: 10}} {...touchableProps}>{row}</RowTouchable>;
  };
...
const styles = StyleSheet.create({
  ...
  dropdown: {
    position: 'absolute',
    // height: (33 + StyleSheet.hairlineWidth) * 5,    // self changed 33 to 38 and 5 to a 2
    borderWidth: StyleSheet.hairlineWidth,
    borderColor: 'lightgray',
    borderRadius: 10, // self change to 10, default 2 rounds option corners on ios
  ...
  list: {
    // flexGrow: 1,
    borderRadius: 10, // self added,
  },
  rowText: {
    paddingHorizontal: 6,
    paddingVertical: 10,
    color: 'black', // self added, changed from gray to black
    backgroundColor: 'white',
    textAlignVertical: 'center',
    borderRadius: 10 // self added to round corners on android
  },
  highlightedRowText: {
    color: 'black',
  },
